...
- CI and PR review automation are active on merge and PR events (`CI` + `PR review (LLM via OpenRouter)`). CI also enforces a **per-file ≥80% line-coverage gate** for both backend (`scripts/check_coverage.py`) and frontend (`npx ng test --no-watch`); merged 2026-06-28 via PR #81.
