...
- User invokes the `@[/function-comments]` skill.
...
- **Domain value objects and records** with business meaning — class-level `<summary>` when the type represents a domain concept that is not obvious from the name alone (e.g. `Resume`, `CoverLetterTemplate`, `Vacancy`).
