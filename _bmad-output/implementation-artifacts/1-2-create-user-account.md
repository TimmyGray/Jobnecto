...
- [ ] Task 3: Implement the application handler and API endpoint for anonymous registration (AC: 1, 5)
  ...
  - [ ] Create `UsersController` with `POST /api/v1/users` and mark the registration endpoint as anonymous so account creation is not blocked by JWT auth.
  - [ ] Implement the handler to validate uniqueness, create the `User` entity, save changes, generate the JWT token, and return the response DTO without password fields.
  - [ ] Set the `Location` header to `/api/v1/users/me` on success.
  - [ ] Preserve the existing global exception handling flow so validation failures still surface as `400` Problem Details and duplicate values surface as `409 Conflict`.
