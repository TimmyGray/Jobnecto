...
## Post-Merge Implementation Status (2026-06-28)

- Completed stories now merged on `master`: `1-1-global-exception-handling`, `1-2-create-user-account`, `1-3-retrieve-current-user-profile`, `1-4-update-user-profile`, `1-5-password-hashing-token-policy-hardening`, `2-1-create-resume`, `2-2-list-resumes`, `2-3-get-resume-detail`, `2-4-update-resume`, `2-5-delete-resume`, `2-6-create-education-record`, `2-7-list-education-records`, `2-8-get-update-delete-education-records`, `r-1-separate-soft-delete-repository-contract`, `3-1-create-cover-letter-template`, `3-2-list-cover-letter-templates`, `3-3-get-cover-letter-template-detail`, `3-4-update-cover-letter-template`, `3-5-delete-cover-letter-template`, `4-1-browse-vacancies-paginated-list`, `4-2-filter-vacancies`, and `4-3-get-vacancy-detail`.
- Implemented API surface includes user registration (`POST /api/v1/users`), token renewal (`POST /api/v1/users/token/refresh`), profile management (`GET /api/v1/users/me`, `PATCH /api/v1/users/me`), avatar endpoints (`POST|PUT|DELETE /api/v1/users/me/avatar`), resume CRUD (`POST /api/v1/resumes`, `GET /api/v1/resumes`, `GET|PATCH|DELETE /api/v1/resumes/{id}`), education CRUD (`POST /api/v1/educations`, `GET /api/v1/educations`, `GET|PATCH|DELETE /api/v1/educations/{id}`), cover letter template full CRUD (`POST|GET /api/v1/cover-letter-templates`, `GET|PATCH|DELETE /api/v1/cover-letter-templates/{id}` with per-user name uniqueness, cursor pagination, and soft-delete), vacancy browse/filter (`POST /api/v1/vacancies/filter` — single keyset-paginated endpoint; `sortBy` supports `createdAt` default, `updatedAt`, and `relevance`; salary range validation; `excludeKeywords` literal exclusion via explicit-escape LIKE), and vacancy detail (`GET /api/v1/vacancies/{id}` with ownership enforcement and soft-delete exclusion).
...
- Epic 2 complete (all resume and education CRUD endpoints delivered). Epic 3 complete (all cover letter template CRUD endpoints delivered). Epic 4 complete: all stories merged — 4.1 (browse/filter vacancies), 4.2 (salary range validation + excludeKeywords), and 4.3 (get vacancy detail). Phase B HTTP core is substantially complete.
- **Phase C (Security) update (2026-05-25):** Phase C is complete. Authentication, password hashing, and ownership enforcement are closed and hardened. Epic R (authorization & ownership hardening, stories r-2 through r-5) merged on `master`, locking the canonical cross-user contract — 404 on detail reads, 403 on mutations, 404 on body-supplied foreign keys — across every authenticated endpoint. The contract is codified in `_bmad-output/planning-artifacts/architecture/authorization-contract-matrix.md`, audited in `_bmad-output/planning-artifacts/architecture/endpoint-ownership-audit.md`, and regression-guarded by `backend/tests/JobNecto.Tests/API/Authorization/`. With Phase C closed, Phase D (Ingestion and LLM) is cleared to start.
- **Phase D kickoff update (2026-06-28):** Story 1.1 (Angular frontend foundation + user registration) is merged to `master` (PR #80). Demo MVP frontend baseline is now live in `frontend/` with standalone Angular app scaffolding, tokenized UI primitives, typed Reactive Forms, OpenAPI-generated TS contracts, RFC 7807 normalization, and sign-up flow integration (`POST /api/v1/users` then `GET /api/v1/users/me` via cookie transport). Phase D is now in-progress.
...
1. All Phase B resume endpoints implemented:
   - Resumes: GET (list), POST (create), GET (single), PATCH (update), DELETE (soft-delete with 204 response)
