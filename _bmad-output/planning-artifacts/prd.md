...
- Epic 2 complete (all resume and education CRUD endpoints delivered). Epic 3 complete (all cover letter template CRUD endpoints delivered). Epic 4 in progress: story 4.1 (browse/filter vacancies — single unified endpoint) complete and merged; stories 4.2 (additional filter ACs: salary range validation, excludeKeywords) and 4.3 (vacancy detail) remain. Phase B HTTP core is substantially complete with user profile, resume, education, cover-letter-template, and vacancy browse/filter support.
