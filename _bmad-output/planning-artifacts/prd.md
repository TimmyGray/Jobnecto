...
- Epic 2 complete (all resume and education CRUD endpoints delivered). Epic 3 complete (all cover letter template CRUD endpoints delivered). Epic 4 in progress: stories 4.1 (browse/filter vacancies — single unified endpoint) and 4.2 (salary range validation + excludeKeywords filter) complete and merged; story 4.3 (vacancy detail) remains. Phase B HTTP core is substantially complete with user profile, resume, education, cover-letter-template, and vacancy browse/filter support.
