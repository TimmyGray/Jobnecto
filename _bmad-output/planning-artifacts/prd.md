...
- Completed stories now merged on `master`: `1-1-global-exception-handling`, `1-2-create-user-account`, `1-3-retrieve-current-user-profile`, `1-4-update-user-profile`, `1-5-password-hashing-token-policy-hardening`, `2-1-create-resume`, `2-2-list-resumes`, `2-3-get-resume-detail`, `2-4-update-resume`, `2-5-delete-resume`, `2-6-create-education-record`, `2-7-list-education-records`, `2-8-get-update-delete-education-records`, `r-1-separate-soft-delete-repository-contract`, `3-1-create-cover-letter-template`, `3-2-list-cover-letter-templates`, `3-3-get-cover-letter-template-detail`, and `3-5-delete-cover-letter-template`.
- Implemented API surface includes user registration (`POST /api/v1/users`), token renewal (`POST /api/v1/users/token/refresh`), profile management (`GET /api/v1/users/me`, `PATCH /api/v1/users/me`), avatar endpoints (`POST|PUT|DELETE /api/v1/users/me/avatar`), resume CRUD (`POST /api/v1/resumes`, `GET /api/v1/resumes`, `GET|PATCH|DELETE /api/v1/resumes/{id}`), education CRUD (`POST /api/v1/educations`, `GET /api/v1/educations`, `GET|PATCH|DELETE /api/v1/educations/{id}`), cover letter template creation (`POST /api/v1/cover-letter-templates` with per-user name uniqueness enforced at the database level, content 50–10,000 characters), cover letter template listing (`GET /api/v1/cover-letter-templates` with cursor pagination and case-insensitive name search, returning `contentPreview` instead of full content), cover letter template detail retrieval (`GET /api/v1/cover-letter-templates/{id}` returning full content for owned templates), and cover letter template soft-delete (`DELETE /api/v1/cover-letter-templates/{id}` returning `204 No Content`; cross-user attempts return `403 Forbidden`).
...
- Epic 2 complete (all resume and education CRUD endpoints delivered). Epic 3 in progress: stories 3.1 (create), 3.2 (list), 3.3 (detail), and 3.5 (delete) complete; story 3.4 (update) remaining. Phase B HTTP core is substantially complete with user profile, resume, education, and cover-letter-template create/list/detail/delete support.
