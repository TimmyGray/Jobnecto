...
- The application now has a complete resume vertical slice: `POST /api/v1/resumes` (create), `GET /api/v1/resumes` (list), `GET /api/v1/resumes/{id}` (detail), and `PUT /api/v1/resumes/{id}` (update) with MediatR command/query handling, FluentValidation, ownership-based 404 responses, and mapper-based DTO-to-entity conversion.
