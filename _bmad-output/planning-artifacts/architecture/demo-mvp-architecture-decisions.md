...
- **Implementation status:** `/sign-in` shipped unguarded and lazy-loaded as Story 1.3, merged 2026-09-25 (PR #88). Guard infrastructure (`authGuard`), the 401 re-auth interceptor, and `returnUrl` intended-destination return shipped as Story 1.4, merged 2026-09-25 (PR #90) — the 401-redirect landed as a separate app-layer interceptor rather than inside the shared HTTP interceptor, to respect this document's own feature-sliced layering (`shared` must not depend on the `entities`-layer `UserService`); see the Story 1.4 file's Completion Notes for the full rationale. The **full route table** (`/dashboard`, `/profile`, `/resumes(/:id)`, `/educations(/:id)`, `/vacancies(/:id)`, `/cover-letters(/:id)`, `/settings`) shipped as Story 1.5, merged 2026-09-26 (PR #92) — `authGuard` was moved from the individual `dashboard` route onto a new shell parent route (`AppShellComponent`) that all of the above are children of, rather than repeated per route; every route whose real page is still backlog (Epics 2-6) points at a shared `ComingSoonStubPage` so no shell nav destination is a dead end.
