...
- **Frontend baseline**: Story 1.1 (Angular frontend foundation + user registration) is merged; the `frontend/` workspace now provides the Demo MVP client scaffold, tokenized UI layer, and sign-up integration against `/api/v1/users` + `/api/v1/users/me`. Story 1.3 (sign-in screen) is also merged: an unguarded `/sign-in` route against `POST /api/v1/users/sessions`. Story 1.4 (session continuity, route guards & expiry recovery) is merged: every other route is now guarded, a silent session restore runs `POST /api/v1/users/token/refresh` on a cold load, and a mid-task `401` redirects to `/sign-in` preserving the intended destination. Story 1.5 (application shell & navigation) is merged: a fixed left sidebar on desktop (top bar + off-canvas drawer on mobile) now wraps every authenticated route, `authGuard` sits once on the shell's parent route, and the full route table from the routing decision below is registered — unbuilt features render a minimal "coming soon" stub rather than a dead-end.
